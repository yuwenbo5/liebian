...
class Admin extends Base
{
    ...
    public function save($id = '')
    {
        ...
        //创建目录
        $path = ROOT_PATH . 'public_html' . DS . $dir . DS . $data['key'];
    ...
    public function ewmuploads($key = "")
    {
        ...
        $dir = config('shequn_file.dir');
        $imgpath1 = ROOT_PATH . 'public_html' . DS . $dir . DS . $key;
        if (!file_exists($imgpath1)) {
            mkdir($imgpath1, 0755);
        }
        $imgpath2 = ROOT_PATH . 'public_html' . DS . $dir . DS . $key . DS . config('shequn_file.ewms');
...
//        halt(count($files));
    ...
    public function tongji($id)
    {
        ...
        $key = $qun['key'];
        $files = glob(ROOT_PATH . "public_html/" . config('shequn_file.dir') . "/" . $key . "/" . config('shequn_file.ewms') . "/*.jpg");
        $num_arr = [];
...
//        halt($data);
    ...
    public function img($qun_id, $ewm_id, $is = 0)
    {
        ...
        record_log('用户【'.$admin['name'].'】('.$admin['username'].')查看【'.$qun['name'].'】线【'.$ewm_id.'】二维码！');
        echo "<img src='$ewm'>";
    }
...
//        halt($data);
     ...
     * @return \think\response\Redirect
     */
    public function delpic($qun_id, $ewm_id)
    {
        $key = DB::name('qun')->field('key')->where('id ='.$qun_id)->find();
        $filename = '.' . DS .  config('shequn_file.dir') . DS . $key['key'] . DS . config('shequn_file.ewms') . DS . $ewm_id .'.jpg';
    ...
    {
        $data = input('post.');
        $key = DB::name('qun')->field('key')->where('id ='.$data['qun_id'])->find();
        $files = glob(ROOT_PATH . "public_html/" . config('shequn_file.dir') . "/" . $key['key'] . "/" . config('shequn_file.ewms') . "/*.jpg");
        $num_arr = [];
        foreach ($files as $k => $v) {
            $ewm = explode("/", $v);
            list($num,) = explode(".", end($ewm));
        ...
        foreach($num_arr as $k=>$v){
            $filename = '.' . DS .  config('shequn_file.dir') . DS . $key['key'] . DS . config('shequn_file.ewms') . DS . $v .'.jpg';
            if(file_exists($filename)){
              $res=unlink($filename);
            }
        }
...
//        input('openid') && $where['openid'] = trim(input('openid'));
    ...
    public function mc($id = 1, $rk = 0)
    {
        ...
        if($rk){
            ...
            echo '<a href="http://qr.liantu.com/api.php?m=10&w=300&text='.$url.'" target="_blank">'.$url.'</a>';
        }else{
            $qun = $quns->get_qun($id);
            dump($qun);
    ...
    public function reurl($rk=""){
      ...
      if($rk){
          ...
          $qun = $quns->get_qun($rukou['qun_id']);
          $qun_id = enXcrypt($rukou['qun_id']);
          $tz=DB::name('QunHosts')->field('name')->where('type','302')->find();

          if($tz){
            if($rukou['host200'] != $tz['name']){
              $rukou['host200']=$tz['name'];
            }
          }
