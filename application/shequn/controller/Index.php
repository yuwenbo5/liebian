...
class Index extends Controller
{
    ...
    public function index()
    {
        ...
        $this->qun_shunt = $rukous->get_qun_shunturl($this->qun['id'],$rokou);
		$qun_shunt=json_decode(json_encode($this->qun_shunt));
    	if($qun_shunt->status =='0'){
			header('Location:'.$qun_shunt->shunturl);
    		exit;
		}
        ...
        //区域
        $area = GetIpAddress($ip) ? GetIpAddress($ip) : "国外";
        $areas = array_merge(explode(',', $this->qun['black_area']), ["国外", "美国", "韩国", "日本", "加拿大", "俄罗斯", "英国", "德国", "法国", "泰国", "澳大利亚", "西班牙"]);
        ...
        $ua = getua();
        $inter = $ua['net'];
        $inters = array_merge(explode(',', $this->qun['internet_name']));

        if (
            in_array($rokou, $qun_rukous) //入口存在
            && $ua['sys']  //能获取设备
            && $ua['net']  //能获取网络
            && !in_array($inter, $inters) //网络加入黑名单
