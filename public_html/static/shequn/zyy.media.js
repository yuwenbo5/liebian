 ...
 *
 * Copyright 2016-2017, iReader FE(æŽŒé˜…ä¹¦åŸŽç ”å‘--å‰ç«¯ç»„)
 * License: MIT
 * 
 */
;
(function() {
	...
	function timeFormat(time, options) {
		// Video's duration is Infinity in GiONEE(é‡‘ç«‹) device
	...
	zyMedia.MediaPlayer = function(media, option) {
		...
		if (t.options.nativeControls || zyMedia.features.isVendorControls) {
			...
			//if (detectType(t.media, t.options, src)) {
				...
				t.init()
			//} else {
			//	alert('ä¸æ”¯æŒæ­¤' + (t.isVideo ? 'è§†' : 'éŸ³') + 'é¢‘')
			//}
		}
	};
	...
	zyMedia.MediaPlayer.prototype = {
		...
		updateTimeline: function(e) {
			...
			if (percent !== null) {
				...
				// Adjust when pause change from playing (é­…æ—)
				if (t.media.paused) {
					setTimeout(function() {
						t.loaded.style.width = _W * percent + 'px';
						t.updateTimeline()
		...
		buildContainer: function() {
			...
			t.container.style.height = (t.isVideo ? _css(t.container, 'width') / t.options.aspectRation : t.options.audioHeight) + 'rem';
		...
		buildDec: function() {
			...
			// some Android device can't fire 'canplay' or irregular working when use 'createEvent' to trigger 'canplay' (è¯»è€…i800)
		...
		init: function() {
			...
			if (t.isVideo) {
				...
				t.media.addEventListener('loadedmetadata', function(e) {
					if (t.enableAutoSize) {
						// For more properly videoWidth or videoHeight of HM 1SW(å°ç±³), QQ browser is 0
						setTimeout(function() {
